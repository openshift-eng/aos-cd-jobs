        ...
        stage( 'operation' ) {
            sshagent([CLUSTER_NAME]) {
                if ( OPERATION == "reinstall" ) {
                    sh "ssh -t -o StrictHostKeyChecking=no opsmedic@use-tower2.ops.rhcloud.com delete"
                    sh "ssh -t -o StrictHostKeyChecking=no opsmedic@use-tower2.ops.rhcloud.com install"
                } else {
                    // Use of -t necessary for Jenkins job termination to ripple down to cicd-control.sh
                    sh "ssh -t -o StrictHostKeyChecking=no opsmedic@use-tower2.ops.rhcloud.com ${OPERATION}"
                }
            }
        }
