...
class PromotePipeline:
    ...
    def check_environment_variables(self):
        ...
        def _check_env_var(env_var: str):
            if not os.environ.get(env_var):
                ...
                if not self.runtime.dry_run:
                    ...
                    logger.warning(msg)
        for env_var in ("GITHUB_TOKEN", "JIRA_TOKEN", "QUAY_PASSWORD"):
            _check_env_var(env_var)
        if not self.skip_signing:
            _check_env_var("SIGNING_CERT")
            _check_env_var("SIGNING_KEY")
    ...
    async def sign_artifacts(self, release_name: str, client_type: str, release_infos: Dict, message_digests: List[str]):
        ...
        try:
            async with AsyncSignatory(uri, cert_file, key_file, sig_keyname=sig_keyname) as signatory:
                tasks = []
                for _, release_info in release_infos.items():
                    pullspec = release_info["image"]
                    digest = release_info["digest"]
        ...
        await asyncio.gather(*tasks)
        self._logger.info("All signatures are published.")
    ...
    async def _publish_json_digest_signatures(self, local_dir: Union[str, Path], env: str = "prod"):
        ...
        if mirror_release_path == "release":
            tasks.append(util.mirror_to_s3(local_dir, f"s3://art-srv-enterprise/pub/openshift-v4/signatures/openshift-release-dev/ocp-release/", exclude="*", include="sha256=*", dry_run=self.runtime.dry_run))
            tasks.append(util.mirror_to_s3(local_dir, f"s3://art-srv-enterprise/pub/openshift-v4/signatures/openshift-release-dev/ocp-release-nightly/", exclude="*", include="sha256=*", dry_run=self.runtime.dry_run))
