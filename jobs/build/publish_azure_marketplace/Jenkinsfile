...
node {
    ...
    stage('upload vhd.gz') {
        ...
        slackChannel.task("Uploading ${params.VERSION} RHCOS VHD for Azure Marketplace") {
            ...
            if (azureArtifactCoordinate.contains(params.VERSION)) {
            ...
            withCredentials([string(credentialsId: 'azure_marketplace_staging_upload_key', variable: '$ACCESS_KEY')]) {
                ...
                echo 'Using jq expression base: ${jq}'
                VHD_GZ_URL=`curl --fail '${rhcosJsonURL}' | jq '${jqExpression}.location' -r`
                VHD_SIG_URL=`curl --fail '${rhcosJsonURL}' | jq '${jqExpression}.signature' -r`
                """ +
                '''
                VHD_SIG_FILE=`basename $VHD_SIG_URL`
                curl --fail  $VHD_SIG_URL > $VHD_SIG_FILE`
                echo Target signature: $VHD_SIG_FILE
                ...
                ''' + params.DRY_RUN?"echo Exiting before upload because of DRY_RUN": '''
                                az storage blob upload -f $VHD_SIG_FILE --container-name rhcos --account-name artupload --account-key $ACCESS_KEY
