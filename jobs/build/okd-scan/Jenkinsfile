...
timeout(activity: true, time: 60, unit: 'MINUTES') {
    node() {
            ...
            [
                ...
                [
                    ...
                    parameterDefinitions: [
                        ...
                        commonlib.okdVersionParam('VERSION'),
                        string(
                            name: 'ASSEMBLY',
                    ...
                    string(
                        name: 'IMAGE_LIST',
                        description: '(Optional) Comma/space-separated list of images to scan (empty to scan all)',
                        defaultValue: "",
                        trim: true,
                    ),
                    booleanParam(
        ...
        stage("Initialize") {
            currentBuild.displayName = "#${currentBuild.number} OKD ${params.VERSION}"
                    ...
                    withCredentials([
                        ...
                        withEnv(["BUILD_USER_EMAIL=${builderEmail?: ''}", "BUILD_URL=${BUILD_URL}", "JOB_NAME=${JOB_NAME}"]) {
                            try {
                                ...
                                echo "Error running OKD ${params.VERSION} scan:\n${err}"
                                throw err

                            } finally {
                                // If scan was skipped, there won't be any logs to archive
                                if (fileExists(doozer_working)) {
                                    sh "mv ${doozer_working}/debug.log ${doozer_working}/debug-okd-${params.VERSION}.log"
                                    sh "bzip2 ${doozer_working}/debug-okd-${params.VERSION}.log"
