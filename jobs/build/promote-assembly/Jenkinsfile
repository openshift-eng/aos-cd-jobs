...
node {
    ...
    } else if (release_info.type == "preview") {
        client_type == "ocp-next"
    }
    ...
    stage("channel prs") {
        if (release_info.type in ["custom", "preview"]) {
            echo "Skipping PR creation for custom (hotfix) and 'preview' release."
            return
        }
