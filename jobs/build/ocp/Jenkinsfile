...
Puddle (internal): http://download-node-02.eng.bos.redhat.com/rcm-guest/puddles/RHAOS/AtomicOpenShift/${version.substring(0,3)}/${OCP_PUDDLE}
  ...
  - Images have been pushed to registry.reg-aws.openshift.com:443/online
...
node(TARGET_NODE) {
    ...
    try {
        sshagent([SSH_KEY_ID]) { // To work on real repos, buildlib operations must run with the permissions of openshift-bot
            ...
            stage( "signing rpms" ) {
                if ( SIGN_RPMS ) {
                    sh "${WORKSPACE}/build-scripts/sign_rpms.sh rhaos-${OSE_VERSION}-rhel-7 openshifthosted"
                } else {
            ...
            stage( "update dist-git" ) {
                sh "ose_images.sh --user ocp-build update_docker --branch rhaos-${BUILD_VERSION}-rhel-7 --group base --force --release '${NEW_RELEASE}-1' --version 'v${spec.version}'"
            }
            ...
            if ( EARLY_LATEST_HACK.asBoolean() ) {
            ...
            stage( "push images" ) {
                dir( "${WORKSPACE}/build-scripts/ose_images" ) {
                    ...
                    sh "sudo ./ose_images.sh --user ocp-build push_images ${TAG_LATEST} --branch rhaos-${BUILD_VERSION}-rhel-7 --group base --push_reg registry.reg-aws.openshift.com:443/online"
                }
            }
