...
OSE-4.6-RHEL-8/84693/release/logging-fluentd-container-v4.6.0-202111191944.p0.gf73a1dd.assembly.stream.tar.gz
        ...
        actual = self.loop.run_until_complete(pipeline._create_tarball_sources([10000, 10001], "fake-working/sources"))
        self.assertEqual(actual, expected)

    @patch("pyartcd.exectools.cmd_assert_async")
    def test_copy_to_spmm_utils(self, cmd_assert_async: AsyncMock):
        cmd_assert_async.return_value = (0, "whatever", "whatever")
        pipeline = TarballSourcesPipeline(MagicMock(dry_run=False), "fake-group-4.10", "fake-assembly", ["fake-component"], [])
        self.loop.run_until_complete(pipeline._copy_to_spmm_utils("fake-working/sources"))
        cmd_assert_async.assert_awaited_once_with(["rsync", "-avz", "--no-perms", "--no-owner", "--omit-dir-times", "--no-group", "fake-working/sources", ANY])
